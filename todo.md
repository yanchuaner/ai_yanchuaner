...
- [ ] 提取会话列表、创建、读取、消息保存、导出、删除和取消生成动作。
- [ ] 将 SSE 读取转换为类型化 UI 事件，组件只消费状态投影。
- [ ] 覆盖空会话、断流、取消、重复提交、会话撤销和恢复测试。
...
- [ ] 提取媒体设置、视觉、图片生成、ASR 和 TTS client/action。
- [ ] 明确浏览器设备处理与服务端代理边界，保持 BYOK 凭据域独立。
- [ ] 覆盖删除凭据、上游拒绝、超时、SSRF 拒绝、格式错误和文本聊天降级。
...
- [ ] `page.tsx` 只负责视图组合、局部 UI 状态和调用领域 action。
- [ ] 对话、角色、世界、资料、账户和媒体各有独立状态边界，不以降低行数为目的制造空壳封装。
- [ ] 桌面和移动端复用相同 action，覆盖加载、空、错误、权限不足、额度不足和取消状态。
