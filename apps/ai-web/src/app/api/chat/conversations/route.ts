...
export async function POST(request: NextRequest) {
  ...
  if (body && typeof body === "object" && !Array.isArray(body)) {
    ...
    if (candidate.mode !== undefined) {
      if (candidate.mode !== "chat" && candidate.mode !== "roleplay") {
  ...
  try {
    const conversation = await createConversation(guard.session.subject.userId, { mode, persona });
    return NextResponse.json({ conversation });
  } catch (error) {
    const message = error instanceof Error ? error.message : "";
    return NextResponse.json(
      { error: message === "persona is invalid" ? "角色设定无效。" : "创建会话失败。" },
      { status: 400 },
    );
  }
}
