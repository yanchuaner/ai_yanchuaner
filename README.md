...
当前阶段以内部验证和真实使用为目标，不建设公开售卖型 API 中转站。
...
- 支持校友自带模型凭据（BYOK）以及校友会提供的公益共享额度。
...
| 角色 | 主要能力 |
| --- | --- |
| 校友与师生 | 使用获准开放的 AI 工具，管理个人偏好与自带凭据 |
...
4. 系统按用户记录用量，执行预算、模型权限和频率限制。
5. 用户可选择使用校友会公益额度或添加自己的 API 凭据。
...
项目与现有燕中网站保持独立仓库。燕中网站负责身份与入口，本项目负责 AI 产品能力和模型访问；两者通过受控接口集成。
...
- 燕中 AI 测试界面：`http://localhost:3000`
