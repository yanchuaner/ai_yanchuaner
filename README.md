...
截至 2026 年 8 月 13 日，自主 `apps/ai-web` 已上线 `ai.yanchuaner.cn`：主站 OIDC、加密 HttpOnly 会话、YanCore 主体交换、DeepSeek 真实对话、额度流水与审计均已实跑。工作台采用工作台首页、角色库、聊天舞台三视图：普通助手、角色扮演与多人群聊三种模式；角色卡可自定场景、世界观和剧情，支持详情预览、收藏、编辑、复制与删除；会话支持置顶、归档、重命名、按角色筛选与 Markdown 渲染。角色资料库与用户级“我的资料”支持粘贴或上传剧情、背景与经历，对话时经向量检索自动注入相关片段；角色长期记忆在对话满 15 条后自动整理摘要，并跨会话带入同角色对话，可查询与清除。多人群聊支持 2–4 位角色同台、可选导演旁白，并按成员分别检索资料。嵌入请求走 api.* 记账，SiliconFlow `BAAI/bge-m3` 渠道已在 api.* 配置并通过真实调用验收。Open WebUI 与 LiteLLM 仅保留内网过渡，不再作为公网产品入口；失败退款、request ID 对账与跨渠道故障切换仍待完成。
...
- 提供普通助手与可 DIY 的角色扮演两种对话模式，自定义角色卡存入个人角色库，不依赖第三方前端。
